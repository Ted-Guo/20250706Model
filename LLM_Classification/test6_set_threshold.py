# -*- coding: utf-8 -*-
"""
Created on Tue Jul 15 14:46:14 2025
...
=== 模擬完整預測（融合兩階段，自訂門檻） ===
Overall Accuracy: 0.7034987908206761
              precision    recall  f1-score   support

           A       0.77      0.64      0.70     20064
           B       0.78      0.63      0.69     19652
         tie       0.61      0.86      0.71     17761

    accuracy                           0.70     57477
   macro avg       0.72      0.71      0.70     57477
weighted avg       0.72      0.70      0.70     57477
...
# 拿到 tie 類機率（class 1 的機率）
tie_proba = clf1.predict_proba(X_all_reset)[:, 1];

final_preds = [];
for i, prob in enumerate(tie_proba):
    if prob > TIE_THRESHOLD:
